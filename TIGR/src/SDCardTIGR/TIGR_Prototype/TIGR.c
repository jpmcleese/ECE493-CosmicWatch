...
// Changelog: 
// - Added UART debug message to display what would be written onto an SD CARD (will be removed in final iteration)
// - Fixed Buffer not correctly saving muon event
// - Fixed Timer to correctly display the date starting from 10/9/2025 12PM
// - Note: We will will need to use a hex editor to view the raw data on the SD as we are not using a FATS library (memory and power intensive)
...
void msp_init(void) {
    ...
    RTCYEAR = 0x2025;                       // Year = 2025 (BCD)
    RTCMON = 0x10;                          // Month = September (BCD)
    RTCDAY = 0x09;                          // Day = 11 (BCD)
    RTCDOW = 0x04;                          // Day of week = Thursday
    RTCHOUR = 0x12;                         // Hour = 12 (BCD)
    RTCMIN = 0x00;                          // Minute = 00 (BCD)
    RTCSEC = 0x00;                          // Seconds = 00 (BCD)
...
int main(void) {
    ...
    UART1string("\r\n\r\n");
    UART1string("***************************************\r\n");
    UART1string("*   TIGR - Radiation Detector v2.1   *\r\n");
    ...
    UART1string("Minute: 0x");
    bcd_to_string(RTCMIN, rtc_str);
    UART1string((unsigned char*)rtc_str);
    UART1string("\r\n");
    
    UART1string("Second: 0x");
